...
#undef DEBUG
	...
	auto done = [&, errorTolerance]() -> bool {
		// done function
		bool pastHalfway = (abs(angleDiff) < abs(initialDistance) / 2);
		bool withinTolerance = (abs(angleDiff) < abs(errorTolerance));
		bool stoppedMoving = abs(currentAngVel) < 0.05 * MAX_TURN_SPEED_CONST;
